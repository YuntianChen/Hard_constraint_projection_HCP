...
weight=0.8
Ss=0.0001
x=np.arange(1,config.nx+1,1)
x=x*20
y=np.arange(1,config.nx+1,1)
y=y*20
...
for i_k in range(n_logk):
    ...
    #4 更改初始条件
    Initial_head_left = 202
    Initial_head_right = 200
    ini = []
    for row in range(config.ny):
        for row_index in range(config.nx // 11):  ##一行最多存11个
            if row_index == 0:
                st = '       ' + str(Initial_head_left) + '.0000000       '
                for num in range(9):
                    st += str(Initial_head_right) + '.0000000       '
                st += str(Initial_head_right) + '.0000000'
            if row_index != 0:
                st = str(Initial_head_right) + '.0000000       '
                for num in range(9):
                ...
                st += str(Initial_head_right) + '.0000000'
            ini.append(st)
        if config.nx - (config.nx // 11) * 11 - 1 != 0:
            st = str(Initial_head_right) + '.0000000       '
            for row_index in range((config.nx - (config.nx // 11) * 11) - 2):
                st += str(Initial_head_right) + '.0000000       '
            st += str(Initial_head_right) + '.0000000'
        ini.append(st)
    f_write = open("initial_head.dat", 'w')
    for i_x in range(len(ini)):
        f_write.write(ini[i_x] + '\n')
    f_write.close()
    ...
    for row in range(config.ny):
        ...
        st = '  ' + str(No_flow) + '   '
        for num in range(config.nx - 3):
            st += str(Flow) + '   '
        st += str(Flow) + '  '
        st += str(No_flow)
        boundary.append(st)
    f_write = open("boundary.dat", 'w')
    for i_x in range(len(boundary)):
        f_write.write(boundary[i_x] + '\n')
    f_write.close()
...
t_train=18
hh=np.vstack((hh[0:t_train,:,:],hh_origion[t_train:nt,:,:]))
